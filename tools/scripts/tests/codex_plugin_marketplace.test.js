...
const pluginEntry = marketplace.plugins.find((plugin) => plugin.name === "ai-skills");
assert.ok(pluginEntry, "marketplace.json must include the ai-skills plugin entry");
assert.deepStrictEqual(
  pluginEntry.source,
  {
    source: "local",
    path: "./plugins/ai-skills",
  },
...
const pluginRoot = path.join(projectRoot, "plugins", "ai-skills");
const pluginManifestPath = path.join(pluginRoot, ".codex-plugin", "plugin.json");
const pluginManifest = JSON.parse(fs.readFileSync(pluginManifestPath, "utf8"));

assert.strictEqual(pluginManifest.name, "ai-skills");
