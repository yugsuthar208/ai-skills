...
**从这里开始：** [AAS Core](users/aas-core.md) · [为仓库加星](https://github.com/yug/ai-skills/stargazers) · [1 分钟安装](#installation) · [选择你的工具](#choose-your-tool) · [按工具查看最佳技能](#best-skills-by-tool) · [捆绑包](users/bundles.md) · [工作流](users/workflows.md)

[![GitHub stars](https://img.shields.io/badge/⭐%2035%2C000%2B%20Stars-gold?style=for-the-badge)](https://github.com/yug/ai-skills/stargazers)
...
- **可安装，而不仅是启发式**：使用 `npx ai-skills` 将技能放置在你的工具期望的位置。
...
使用 `npx ai-skills`，或工具特定标志，如 `--codex`、`--cursor`、`--gemini`、`--claude`、`--antigravity` 或 `--agy`，当你希望安装器定位到特定技能目录时。对于上下文敏感的宿主，还可以用 `--risk`、`--category` 和 `--tags` 安装较小子集。
...
| **Copilot**     | Ext（`gh skill` 预览） | `gh skill preview yug/ai-skills skills/brainstorming/SKILL.md` | GitHub CLI `gh skill` |
...
| 工具           | 安装                                                                  | 首次使用                                              |
| -------------- | ------------------------------------------------------------------------ | ------------------------------------------------------ |
| Claude Code    | `npx ai-skills --claude` 或 Claude 插件市场 | `>> /brainstorming help me plan a feature`           |
...
| Codex CLI      | `npx ai-skills --codex`                               | `Use brainstorming to plan a feature`                |
| Antigravity    | `npx ai-skills --antigravity`                         | `Use @brainstorming to plan a feature`               |
...
| GitHub Copilot | `gh skill install yug/ai-skills skills/brainstorming/SKILL.md --agent github-copilot --scope user --pin v14.2.0`（预览） | `Ask Copilot to use brainstorming to plan a feature` |
...
| 自定义路径    | `npx ai-skills --path ./my-skills`                    | 取决于你的工具                                   |
...
- **[Agentic Awesome Skills vs Awesome Claude Skills](users/ai-skills-vs-awesome-claude-skills.md)** 了解广度 vs 精选列表的权衡。
...
```bash
git clone https://github.com/yug/ai-skills.git .agent/skills
```

如果你有在移除的符号链接变通方法附近创建的旧克隆，请重新安装到新目录或重新运行 `npx ai-skills` 安装器。
...
- 清理前的备份路径
- 通常需要清除的存储文件夹
- 根据 [issue #274](https://github.com/yug/ai-skills/issues/274) 改编的可选批处理助手
...
**在线托管：**相同的应用可在 [https://yug.github.io/ai-skills/](https://yug.github.io/ai-skills/) 获得，并在每次推送到 `main` 时自动部署。要启用一次：**设置 → 页面 → 构建和部署 → 源：GitHub Actions**。
...
- [Discussions](https://github.com/yug/ai-skills/discussions) 用于提问、想法、展示帖子和社区反馈。
- [Issues](https://github.com/yug/ai-skills/issues) 用于可重现的错误和具体的、可操作的改进请求。
- [在 X 上关注 @yug](https://x.com/yug) 获取项目更新和发布。
...
- [在 Buy Me a Coffee 上给我买一本书](https://buymeacoffee.com/yug)
- 为仓库加星
- 提出可重现的问题
- 贡献文档、修复和技能
...
<a href="https://github.com/yug/ai-skills/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=yug/ai-skills" alt="Repository contributors" />
</a>

Made with [contrib.rocks](https://contrib.rocks)。*（图像可能已缓存；[在 GitHub 上查看实时贡献者](https://github.com/yug/ai-skills/graphs/contributors)。)*
...
[![yug/ai-skills - Star History Chart](https://api.star-history.com/image?repos=yug/ai-skills&style=landscape1)](https://star-history.com/yug/ai-skills)

[![Star History Chart](https://api.star-history.com/svg?repos=yug/ai-skills&type=date&legend=top-left)](https://www.star-history.com/#yug/ai-skills&type=date&legend=top-left)
