...
curl --fail --silent --show-error --request POST \
  --header 'User-Agent: ai-skills:claude-code' \
...
Set the User-Agent to `ai-skills:{HARNESS/LLM_NAME}`, where the second field is the agent or model you are running as (`claude-code`, `cursor`, `codex`, `gemini-cli`). A non-default User-Agent is required (see Security & Safety Notes); an identifying one lets Cohesivity attribute the request.
...
- **Problem:** HTTP 403 with "error 1010" on every request.
  **Solution:** The HTTP client is sending a default User-Agent. Set `ai-skills:{HARNESS/LLM_NAME}`.
