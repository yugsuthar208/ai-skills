...
The package publishes separate `ai-skills`, `aas`, and `aas-mcp` binaries. Use the explicit `aas` binary for Core lifecycle commands; the legacy `ai-skills` entrypoint remains the direct installer.

### Where should I install the skills?

It depends on how you install:

- **Using the installer CLI (`npx ai-skills`)**:
...
If you get a 404 from npm, use: `npx github:yug/ai-skills`
...
set with `npx ai-skills --antigravity --skills <ids> --dry-run`.
AAS MCP selects and validates IDs but does not install them. The complete catalog
requires the explicit `npx ai-skills --antigravity --all` override
...
- an optional batch script adapted from [issue #274](https://github.com/yug/ai-skills/issues/274)
...
Please [Open an issue](https://github.com/yug/ai-skills/issues)!
Include:
...
**Still confused?** [Open a discussion](https://github.com/yug/ai-skills/discussions) and we'll help you out! 🙌
